class Award extends ValidatedListenedEntity {
    ...
    async validate(property, from, to) {
        ...
        if (this.rank < this.template.ranks.from)
            await this.setListenedProperty('rank', this.template.ranks.from);
        else if (this.rank > this.template.ranks.to)
            await this.setListenedProperty('rank', this.template.ranks.to);
    ...
    constructor(params = {}) {
        ...
        if (Award.Orders[this.order].variants && !this.variant)
            this.variant = Award.Orders[this.order].variants[0];
    }
    get fields() {
    ...
    v_variant;
    get variant() { return this.v_variant; }
    set variant(value) {
        this.setListenedProperty('variant', value);
    }
    v_reason = '';
    get reason() { return this.v_reason; }
    set reason(value) {
        this.setListenedProperty('reason', value);
    }
    v_flavour;
    get flavour() { return this.v_flavour; }
    set flavour(value) {
        this.setListenedProperty('flavour', value);
    }
    v_player = '';
    get player() { return this.v_player; }
    set player(value) {
    ...
    v_issuer;
    get issuer() { return this.v_issuer; }
    set issuer(value) {
